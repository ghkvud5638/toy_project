...
public class AttractionPaging {
	...
	private int chkNo; // 리스트 몇개 할지 생각중임니다
	
	

	public AttractionPaging() { }
	
	//총 게시글 수만 입력하는 생성자
	public AttractionPaging(int totalCount) {
...
//		this.setChkNo(3);
	...
	public AttractionPaging(int totalCount, int curPage, int chkNo) {
		this.setTotalCount(totalCount);
	...
	public AttractionPaging(int totalCount, int curPage, int listCount, int chkNo) {
		this.setTotalCount(totalCount);
		this.setCurPage(curPage);
	...
	public AttractionPaging(int totalCount, int curPage, int listCount, int pageCount, int chkNo) {
		this.setTotalCount(totalCount);
		this.setCurPage(curPage);
		this.setListCount(listCount);
	...
	private void makePaging() {
		...
		if(listCount==0)	setListCount(chkNo);//화면에 보여질 게시글수 기본값 세팅
	...
	public String toString() {
		return "AttractionPaging [curPage=" + curPage + ", totalCount=" + totalCount + ", listCount=" + listCount
				+ ", totalPage=" + totalPage + ", pageCount=" + pageCount + ", startPage=" + startPage + ", endPage="
				+ endPage + ", startNo=" + startNo + ", endNo=" + endNo + ", chkNo=" + chkNo + "]";
	}
